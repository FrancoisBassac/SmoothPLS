---
title: "comparison_one_state_PFR"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{comparison_one_state_02}
...
nbasis = 100 # number of basis functions
...
```{r plot df individuals, fig.alt="Individuals plot"}
plot_CFD_individuals(df)
```

```{r, fig.alt="Individuals plot by cfda"}
...
```{r hist Y noised, fig.alt="Y real and noised histograms"}
par(mfrow=c(1, 2))
hist(Y_df$Y_real)
hist(Y_df$Y_noised)
par(mfrow=c(1, 1))
...
# Basis creation 
```{r basis creation, fig.alt="Basis function"}
...
```{r, fig.alt="Naive PLS CV and regression coefficients"}
...
```{r, fig.alt="Real function VS naive PLS coefficients"}
...
```
# FPLS
```{r, fig.alt="Functional PLS CV and functional coefficient"}
...
```{r, fig.alt="REal function VS FPLS functional coefficient"}
...
```
# Smooth PLS
```{r, fig.alt="SmoothPLS CV and functional coefficient"}
...
## delta
Delta Smooth_PLS
```{r, fig.alt="SmoothPLs functional coefficient 2"}
...
```{r, fig.alt="Real function VS SmoothPLS coefficient"}
y_lim = eval_max_min_y(f_list = list(beta_real_func, 
                                     delta), 
                       regul_time = regul_time_0)
...
## model
```{r}
pfr_obj = mpfr(df_list = df, Y = Y, basis_obj = basis,
                     curve_type_obj = 'cat', int_mode = 1,
                     print_steps = TRUE,
                     plot_rmsep = TRUE, plot_reg_curves = TRUE,
                     id_col_obj = 'id', time_col_obj = 'time',
                     parallel=TRUE)
...
evaluate_curves_distances(real_f = beta_real_func,
                          ...
                          fun_fd_list = list(
                            pfr_obj$reg_obj$CatFD_1_state_1
                            )
                          )
```
```{r}
y_lim = eval_max_min_y(f_list = list(pfr_obj$reg_obj$CatFD_1_state_1,
                                     beta_real_func ), 
                         regul_time = regul_time_0)

plot(regul_time_0, beta_real_func(regul_time_0), type='l', ylab="Values",
     xlab = 'Time', ylim = y_lim, col = 'black')
title(paste0(names(pfr_obj$reg_obj)[2], " regression curves"))
plot(pfr_obj$reg_obj$CatFD_1_state_1, add=TRUE, col='blue')
legend("topleft",
         legend = c("Real curve", "PFR"),
         col = c("black", "blue"),
         lty = 1,
         lwd = 1)
```
...
```{r}
plot(pfr_obj$modele_final$residuals)
```
...
```{r, fig.alt="Curves comparison, all"}
...
```{r, fig.alt="Curves comparison, real, FPLS, SmoothPLS"}
y_lim = eval_max_min_y(f_list = list(spls_obj$reg_obj$CatFD_1_state_1,
                                     fpls_obj$reg_obj$CatFD_1_state_1,
                                     beta_real_func ), 
                         regul_time = regul_time_0)
...
```{r, fig.alt="FPLS, SmoothPLS secodn derivatives"}
...
```{r, fig.alt="Train error histogram"}
par(mfrow=c(1,2))
...
```{r, fig.alt="Test error histogram"}
par(mfrow=c(1,2))
...
## Results plots
### All
```{r, fig.alt="PLot models metrics"}
plot_model_metrics_base(train_results, test_results)
```

### FPLS & SmoothPLS
```{r, fig.alt="Plot metric FPLS and SmoothPLS"}
plot_model_metrics_base(train_results, test_results,
...
# Fourier basis test
```{r basis creation Fourier, fig.alt="Fourier basis"}
...
## FPLS
```{r, fig.alt="FPLS CV and functional coefficient Fourier"}
...
## Smooth PLS 
```{r, fig.alt="SmoothPLs CV and functional coefficient Fourier"}
...
```{r, fig.alt="Fourier SmoothPLs functional coefficient"}
...
## Curve comparison
```{r, fig.alt="Fourier curve comparison"}
plot(regul_time_0, beta_real_func(regul_time_0), type='l', xlab="Beta_t",
     ylim = c(-2, 3.5))
...
```{r, fig.alt="Fourier all curves comparison"}
...
```{r, fig.alt="Fourier FPLS SmoothPLS vs TRUE function"}
y_lim = eval_max_min_y(f_list = list(spls_obj_2$reg_obj$CatFD_1_state_1,
                                     fpls_obj_2$reg_obj$CatFD_1_state_1,
                                     beta_real_func), 
                         regul_time = regul_time_0)
...
```{r, fig.alt="Fourier plot models metrics"}
plot_model_metrics_base(train_results_f, test_results_f)
```

```{r, fig.alt="Fourier plot metrics FPLS SmoothPLS"}
plot_model_metrics_base(train_results_f, test_results_f,
                        models_to_plot = c('FPLS', 'SmoothPLS'))
```
